...
from ME.dataTest import Dataset, nextBatch
from ME.test import BiLSTM_Att_CNN
from ME.text_cnn import TextCNN
from ME.BLA import BiLSTMAttention
from ME.RCNN import RCNN
...
class Config(object):
    ...
    rate = 0.8  # 训练集的比例
    batchSize = 64
    seq_len = 200  # "Input sequence length."
    n_head = 8
    full_units = 200
...
def train(num):
    ...
    backTrainReviews = data.backTrainReviews
    backEvalReviews = data.backEvalReviews
    backTestReviews = data.backTestReviews
    print(data.labels)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            def trainStep(batchX, batchY, batchXB):
                """
                训练函数
                """
                if num == 1:
            ...
            def devStep(batchX, batchY, batchXB):
                """
                验证函数
                """
                if num == 1:
            ...
            def testStep(batchX, batchY, batchXB):
                """
                验证函数
                """
                if num == 1:
            ...
            for i in range(config.training.epoches):
                ...
                for batchTrain in nextBatch(trainReviews, trainLabels, config.batchSize, backTrain=backTrainReviews):
                    train_step_loss, train_step_acc = trainStep(batchTrain[0], batchTrain[1], batchTrain[2])
                    # currentStep = tf.train.global_step(sess, global_step) # 当前状态
                    # if currentStep % 20 == 0:
                    #     break

                for batchEval in nextBatch(evalReviews, evalLabels, config.batchSize, backTrain=backEvalReviews):
                    eval_step_loss, eval_step_acc = devStep(batchEval[0], batchEval[1], batchTrain[2])
                ...
                preds = []
                y_test = []
                for batchTest in nextBatch(testReviews, testLabels, config.batchSize, backTrain=backTestReviews):
                    test_step_pred, test_step_acc = testStep(batchTest[0], batchTest[1], batchTrain[2])
